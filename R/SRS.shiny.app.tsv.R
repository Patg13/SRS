SRS.shiny.app.tsv <- function(filename, launch_browser=FALSE){
  # Load a TSV file converted from a BIOM file (designed for Qiime2 - Q2Pipe Pipeline
  ...
  # Run the application (launch_browser option is for compatibility purpose (ex: WSL does not have a default browser, but Windows browser can access web server)
  shinyApp(ui = ui, server = server, options = list(launch.browser = launch_browser))}
